// https://github.com/ecnerwala/cp-book/blob/master/src/manacher.hpp
#include <cassert>
#include <vector>
...
/**
 * manacher_odd(S): return the maximum palindromic substring of S centered at
 * each point
 *
 * Input: string (or vector) of length N (no restrictions on character-set)
 * Output: vector res of length N
 *   For any 0 <= i < N:
 *   * the half-open substring S[i-res[i], i+res[i]] is a palindrome of length
 * 2*res[i]+1
 */
